#include <iostream>
#include <vector>
#include <stdexcept>

struct pair {
    int first;
    int second;
};

#define SEPARATOR ' '
template <typename T, typename comparator = std::less<>>
class Heap {
public:
    std::vector<T> data;
    size_t size = 0;
    comparator cmp;

public:
    void Insert(T);
    void SiftUp(size_t);
    void SiftDown(size_t, std::vector<pair>&);
    T Top();
    T Pop();
    size_t Left(size_t);
    size_t Parent(size_t);
    size_t Right(size_t);

    T operator [] (size_t);
    size_t Size();

};

template <typename T, typename comparator>
size_t Heap<T, comparator>::Left(size_t i) {
    size_t res = 2 * i + 1;
    if (res >= size) throw std::out_of_range("There is no left child");
    return res;
}

template <typename T, typename comparator>
size_t Heap<T, comparator>::Right(size_t i) {
    size_t res = 2 * i + 2;
    if (res >= size) throw std::out_of_range("There is no right child");
    return res;
}

template <typename T, typename comparator>
size_t Heap<T, comparator>::Parent(size_t i) {
    if (i == (size_t)0) throw std::out_of_range("There is no parent");
    size_t res = (i - 1) / 2;
    return res;
}

template <typename T, typename comparator>
void Heap<T, comparator>::Insert(T value) {
    data.push_back(value);
    SiftUp(size++);
}

template <typename T, typename comparator>
void Heap<T, comparator>::SiftUp(size_t i) {
    while (i > 0 && cmp(data[i], data[(i - 1)/2])) {
        std::cout << i << ' ' << (i - 1)/2 << std::endl;
        std::swap(data[i], data[(i - 1)/2]);
        SiftDown(i);
        i = (i - 1) / 2;
    }
    
}

template <typename T, typename comparator>
T Heap<T, comparator>::operator [] (size_t index) {
    if (index >= size)  throw std::out_of_range("What a misery");
    else return data[index];
}

template <typename T, typename comparator>
std::ostream& operator << (std::ostream& out, Heap<T, comparator> hp) {
    for (size_t i = 0; i != hp.Size(); ++i) {
        if (i != 0) out << SEPARATOR;
        out << hp[i];
    }
    return out;
}

template <typename T, typename comparator>
size_t Heap<T, comparator>::Size() {
    return size;
}

template <typename T, typename comparator>
T Heap<T, comparator>::Top() {
    if (size == 0) throw std::out_of_range("The heap is empty");
    else return data.front();
}

template <typename T, typename comparator>
T Heap<T, comparator>::Pop() {
    if (size == 0) throw std::out_of_range("The heap is empty");
    T elem = data.front();
    std::swap(data.front(), data.back());
    data.resize(--size);
    SiftDown(0);
    return elem;
}

template <typename T, typename comparator>
void Heap<T, comparator>::SiftDown(size_t index, std::vector<pair>& ans) {
    try {
        size_t cur = index;
        try {if (cmp(data[Left(index)], data[cur])) cur = Left(index);} catch (...) {}
        try {if (cmp(data[Right(index)], data[cur])) cur = Right(index);} catch (...) {}
        // std::cout << "\nData:\n";
        // for (size_t i = 0; i != data.size(); ++i) {
        //     std::cout << data[i] << ' ';
        // }
        // std::cout << "\n-----\n";
        if (cur != index) {
            ans.push_back({cur, index});
            std::swap(data[index], data[cur]);
            SiftDown(cur, ans);
        }
    } catch (...) {}
}




int main() {
    int n; std::cin >> n;
    std::vector<long long> v;
    for (int i = 0; i != n; ++i) {
        long long x; std::cin >> x;
        v.push_back(x);
    }
    Heap<int, std::less<int>> hp;
    hp.data.resize(v.size());
    hp.size = v.size();
    std::copy(v.begin(), v.end(), hp.data.begin());
    std::vector<pair> ans;
    for (int i = hp.data.size() - 1; i >= 0; --i) {
        hp.SiftDown(i, ans);
    }
    std::cout << ans.size() << '\n';
    for (int i = 0; i != ans.size(); ++i) {
        std::cout << ans[i].first << ' ' << ans[i].second << '\n';
    }
}